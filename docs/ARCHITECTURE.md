...
packages/
  ...
  classification/         Classification Engine: derives structured metadata (Skills today; technologies, languages, chains, seniority, employment type later) from Opportunities.
  matching/               User Intelligence: User Profiles, Skills, preferences, and deterministic Match computation against Opportunities.
...
- **`packages/scoring`** depends on `packages/events`, `packages/db` (to read the facts it correlates and to persist Opportunities and their explainable scores), and `packages/shared` (generic utilities, e.g. deterministic ID derivation). It never depends on `packages/decision`, `packages/ai`, or `packages/collectors`. It also never depends on `packages/classification` or `packages/matching` — Opportunity scoring and Opportunity metadata classification are deliberately separate concerns (see below), and neither imports the other.
- **`packages/classification`** depends on `packages/events`, `packages/db`, and `packages/shared` — the same shape as `packages/scoring`, for the same reason: it reads Source Events and persists a derived, explainable, append-only fact (`OpportunitySkillDetected` today; further metadata categories are additive later), event-first. It never depends on `packages/scoring`, `packages/decision`, `packages/ai`, `packages/matching`, or `packages/collectors`. Kept separate from `packages/scoring` specifically so Company Intelligence/Opportunity scoring stays focused on *whether and how strongly* something is an Opportunity, while classification stays focused on *what kind* of Opportunity it structurally is — two questions with different rates of change and different taxonomies, per the Milestone 5 refinement.
- **`packages/matching`** depends on `packages/events`, `packages/db`, and `packages/shared`. It owns the User-side counterpart to `packages/scoring`'s Company-side Signal/Intelligence/Opportunity chain: User Profiles, the Skill taxonomy, and deterministic Match computation (User Profile × Opportunity, event-first via `MatchComputed`, then a `match` projection). It never depends on `packages/scoring`, `packages/classification`, `packages/decision`, `packages/ai`, or `packages/collectors` — it reads `opportunity` and `opportunity_skill` rows directly through `packages/db`, the same read pattern `packages/application` uses, rather than importing either package's internals.
- **`packages/decision`** depends on `packages/events`, `packages/db`, and `packages/shared` (deterministic ID derivation for `recommendation` rows, the same pattern every other domain package uses). It reads `match`, `opportunity`, and `company_intelligence` rows directly through `packages/db` — the same read-not-import pattern `packages/matching` uses for `opportunity`/`opportunity_skill` — and never depends on `packages/scoring`, `packages/classification`, or `packages/matching` internals. It never depends on `packages/ai` — a deterministic decision must never be able to reach for a non-deterministic dependency.
- **`packages/ai`** depends on `packages/events`, `packages/db`, and `packages/shared` — the same shape as every other domain package. It reads `recommendation`, `opportunity`, `company`, `company_intelligence`, `signal`, `match`, `user_profile`, and `user_skill` rows directly through `packages/db` (the same read-not-import pattern `packages/matching` and `packages/decision` use), and never depends on `packages/scoring`, `packages/classification`, `packages/matching`, or `packages/decision` internals. It never writes to any table those packages own — only to its own five AI artifact tables.
...
`apps/web → packages/{ai,decision,scoring,classification,matching,notifications,collectors,ingestion,db,ui,shared,events,application}`, and within `packages/`, dependencies point only toward `events`, `ingestion`, `db`, and `shared` — never sideways between domain packages (`collectors` ↛ `scoring`, `scoring` ↛ `decision`, `decision` ↛ `ai`, `classification` ↛ `scoring`, `matching` ↛ `classification`, etc.) and never back up toward `apps/web`. `packages/application` is a special case of this same rule, not an exception to it: it depends only on `packages/db` and `packages/shared`, exactly like a domain package would. `packages/events → packages/db` and `packages/ingestion → {packages/events, packages/db}` are the only exceptions to "nothing depends on `db` except through its own layer," and both are one-way and terminal: `packages/db` never depends on `packages/events` or `packages/ingestion`, and `packages/events` never depends on `packages/ingestion`, so no cycle is introduced. Communication between domain packages happens through events, not imports. A circular dependency between any two packages is treated as a design defect, not a lint warning to suppress.

**Note:** the pipeline order described in [§5](#5-data-flow) (Scoring → Decision → AI) is a sequence of *stages*, not an import chain. `packages/decision` does not import `packages/scoring`, and `packages/ai` does not import `packages/decision` — each depends only on `packages/events`, `packages/db`, and `packages/shared`, and learns what an earlier stage did by reading the projection rows that stage's own package persisted (`match`, `opportunity`, `recommendation`, etc.), the same read-not-import pattern throughout this section — never by importing that package's internals, and never through a live event-bus subscription (this system has none; every pipeline stage today is invoked externally, not triggered by publication). This is what keeps the pipeline reorderable, independently testable, and free of the sideways coupling a naive "engine calls the next engine directly" implementation would introduce.
...
The Classification Engine. Derives structured, explainable metadata from an Opportunity's underlying Source Events — Skills today (`OpportunitySkillDetected`, extracted from `JobPosted` title/department text against the Skill taxonomy), with technologies, languages, chains, seniority, and employment type as additive future classifiers behind the same open registry. Every classification cites the Event(s) it was derived from, the same evidentiary discipline `packages/scoring` applies to Signals. Deliberately separate from `packages/scoring`: scoring answers "is this a real, scored Opportunity," classification answers "what kind of Opportunity is this" — different taxonomies, different rates of change, no reason to be the same package or the same pull request.

### `packages/matching`

User Intelligence. The User-side counterpart to `packages/scoring`'s Company-side Intelligence: owns User Profiles, the Skill taxonomy a User's declared Skills reference, preferences/deal-breakers, and deterministic Match computation against Opportunities (via `packages/classification`'s `opportunity_skill` data). A Match is always computed between a User Profile and an Opportunity — never a User and a Company (Domain Rule #4) — and is event-sourced the same way Company Intelligence is: a `MatchComputed` Event precedes every meaningful change to the `match` projection. Produces a score plus explicit reasoning (matched Skills), never a bare number, per [Match ≠ Score](./DOMAIN_MODEL.md#ubiquitous-language).

### `packages/decision`

The Decision Engine. Consumes deterministic Matches (never raw Events, never Opportunities directly) and applies deterministic, versioned business rules — an open registry of eligibility rules, the same pattern `packages/scoring`'s Signal detectors use — to decide whether a Match becomes a Recommendation, per [Match ≠ Recommendation](./DOMAIN_MODEL.md#ubiquitous-language). Owns the Recommendation's full lifecycle as an explicit state machine (`active` → `dismissed`/`archived`/`expired`, `dismissed`/`archived` → `active`, `expired` terminal) and its priority (Match owns score; Decision owns priority — deterministically derived from Match score, Company Intelligence confidence, freshness, and Recommendation age). Recommendation reasons are stored as structured data (`reasonCode`/`reasonDetails`/`reasonVersion`); rendering them into user-facing wording is `packages/application`'s job, not this package's. Digest candidate selection and follow-up scheduling are pure functions operating on Recommendations, not Matches, and return values rather than persisting new entities. Contains no AI calls and no other non-deterministic dependency — this is what keeps it predictable and independently testable, and what makes "AI never becomes the source of truth" (see [§9](#9-design-constraints)) an enforceable rule rather than a hope.

### `packages/ai`

The AI Layer. The only package permitted to invoke an LLM provider — a provider abstraction (OpenAI, Anthropic, and a deterministic Mock used by every automated test) that all business logic in this package depends on, never a specific provider. Enriches deterministic entities that already exist (Recommendation explanations, Opportunity/Company summaries, outreach drafts, Profile insights) with plain-text, grounded content; it never computes a score, a Match, a classification, or a Recommendation, and nothing in `packages/scoring`, `packages/matching`, `packages/classification`, or `packages/decision` ever reads its output. Generation is always User-triggered (an explicit "Generate" action), never automatic — the deterministic pipeline and every page render identically whether or not AI is configured, which is what makes "AI never becomes the source of truth" (see [§9](#9-design-constraints)) true in production, not just in the dependency graph. AI artifacts are append-only and versioned: `promptVersion` marks which prompt-template revision produced a row, and a template change is a cache miss, not an overwrite — `version` increments instead. Every artifact publishes an Event recording the enrichment (`AIRecommendationGenerated`, `OpportunitySummaryGenerated`, `CompanySummaryGenerated`, `OutreachDraftGenerated`, `ProfileInsightGenerated`) — these map into the *existing* Event category taxonomy by what each artifact is about (`recommendation`, `intelligence`, `intelligence`, `decision`, `user` respectively), not a category of their own: the taxonomy represents business-domain lifecycle, and `packages/ai` is simply another producer, the same way multiple Collectors all publish `source` events.

### `packages/application`

The Application Layer. Sits between `packages/db` and every consumer of persisted data — the public API and `apps/web`'s Server Components — and owns all read-side orchestration: `OpportunityQueryService`, `CompanyQueryService`, and `SearchService` compose the pagination, filtering, sorting, and search queries a browsable feed and detail pages need, and return stable, API-facing DTOs/View Models (e.g. `OpportunityFeedItemDTO`, `OpportunityDetailDTO`, `CompanyProfileDTO`, `SearchResultDTO`) rather than leaking `packages/db` row shapes to any consumer. Read-only: it never writes to `packages/db` and never publishes an event. This is what makes "no UI component or API route composes a database query" an enforceable rule rather than a convention — see [§9](#9-design-constraints). Since Milestone 5, its query services optionally accept a `viewerId` and, when present, read that viewer's `match` row alongside the Opportunity data — still a read of already-materialized `packages/matching` output, never a Match computation itself. Since Milestone 6, `RecommendationQueryService` is the first *authenticated-only* read path this package exposes — unlike the Opportunity Feed's graceful degradation to `viewerId: undefined` for anonymous visitors, a Recommendation belongs to exactly one User and has no public form, so its query functions require a `userId`, not accept one optionally. It also renders `packages/decision`'s structured `reasonCode`/`reasonDetails` into user-facing wording — the Decision Engine owns the structured fact, the Application Layer owns presentation.
...
- **Domain packages do not import each other's internals.** Cross-domain communication happens through `packages/events`, not through direct imports between `collectors`, `scoring`, `classification`, `matching`, `decision`, `ai`, and `notifications`.
...
- **A Recommendation, Match, or User Profile is never visible to any User other than the one it belongs to.** Every read and write path for User-scoped data requires a `userId` resolved from the caller's session — never a client-supplied parameter — and an unknown ID and someone-else's ID must be indistinguishable in the response (both "not found," never a distinguishing error), so a client can't probe for the existence of another User's data.
...
4. ~~**Entity resolution strategy for companies.**~~ **Resolved (Milestone 8): explicit, operator-declared mapping, not fuzzy/heuristic matching.** `company_source_identity` (`packages/db/src/schema/company-source-identity.ts`) records, per Collector, "this source's own identifier for a company (a Greenhouse board token, a Lever site slug, an Ashby job board name) refers to this Company row" — declared by whoever configures a Collector's tracked companies (`apps/web/lib/collectors/run-collector.ts`), never inferred by name/domain matching. Resolution itself still goes through `company.slug`, a canonical, operator-chosen identifier shared across a company's per-source configurations, unchanged since Milestone 2; the new table exists so *why* three sources merged into one Company is an auditable, queryable fact rather than an implicit config coincidence. Insert-once for now — repointing a source identifier to a different Company, and retroactively merging two already-separate Company rows (`DOMAIN_MODEL.md`'s Company "Merged" lifecycle transition), remain distinct, deferred capabilities.
...
9. ~~**Authentication/identity strategy.**~~ **Resolved (Milestone 5): Better Auth**, installed in Milestone 0 (its own `user`/`session`/`account`/`verification` schema, now explicitly wired into `packages/db` rather than left implicit) and given a real sign-up/login surface in Milestone 5. `packages/matching`'s `user_profile` table is the domain-data counterpart, per [DOMAIN_MODEL.md](./DOMAIN_MODEL.md#user-profile)'s User ≠ User Profile distinction — Better Auth's `user` table remains account/identity only.
