...
- **`packages/scoring`** depends on `packages/events` and `packages/db` (to read the facts it correlates and to persist Opportunities and their explainable scores). It never depends on `packages/decision`, `packages/ai`, or `packages/collectors`.
