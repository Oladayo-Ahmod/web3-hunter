...
**Seniority**, checked precisely against the current code (`job-relevance.ts`): `SENIOR_TITLE_KEYWORDS = ["senior", "sr.", "staff", "principal", "lead"]`, `JUNIOR_TITLE_KEYWORDS = ["junior", "jr.", "entry-level", "entry level", "intern"]`. **"Manager" and "Director" are in neither list** — a title like "Director of Engineering" gets `inferSeniorityFromTitle() === null` today, i.e. *no seniority signal at all*, not a mismatch penalty. This is a real, verified gap, not speculation.
